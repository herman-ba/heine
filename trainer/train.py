...
def directional_accuracy(model: LSTMModel, loader: torch.utils.data.DataLoader, device: str) -> float:
    hits = 0
    total = 0
    with torch.inference_mode():
        for x, y in loader:
            ...
            pred = model(x)
            last_close = x[:, -1, 3]
            pred_dir = torch.sign(pred - last_close)
            true_dir = torch.sign(y - last_close)
            mask = torch.isfinite(y)
            hits += (pred_dir[mask] == true_dir[mask]).sum().item()
            total += mask.sum().item()
    ...
    if os.path.exists(best_path):
        ...
        with torch.inference_mode():
            for x, y in val_loader:
                ...
                val_loss += loss.item() * x.size(0)
        val_loss /= len(val_loader.dataset)
        hit_rate = directional_accuracy(model, val_loader, cfg['device'])
        print(
            f"Best validation loss: {val_loss:.6f} at epoch {early_stop.best if early_stop.best < float('inf') else best_epoch}; "
